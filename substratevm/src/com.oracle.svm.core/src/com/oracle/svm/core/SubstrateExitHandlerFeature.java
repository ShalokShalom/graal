...
final class SubstrateExitHandlerStartupHook implements RuntimeSupport.Hook {
    ...
    public void execute(boolean isFirstIsolate) {
        if (!SubstrateOptions.EnableSignalHandling.getValue()) {
            throw new UnsupportedOperationException("java.lang.Terminator can only be set when -XX:+EnableSignalHandling is set. Please enable this option via the command line.");
        }
        if (isFirstIsolate) {
